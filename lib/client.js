			...
			ctx.inject(["inputTriggers"], (scope) => {
				...
				return triggers.registerSource({
					...
					async candidates(_session, req) {
						...
						return (q !== "" ? files.filter((f) => f.name.toLowerCase().includes(q) || f.rel.toLowerCase().includes(q)) : files).slice(0, 50).map((f) => ({
							name: f.name,
					...
					lexicon(_session) {
						if (!activeWorkspaceRoot || fileCache.length === 0) return void 0;
						const paths = [];
