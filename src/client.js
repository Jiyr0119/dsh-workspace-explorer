...
// 职责：右侧浮层面板（对齐 DSH 原生弹窗风格）+ 工作区文件树 + 文件类型图标 + 拖拽/点击插入引用。
...
.dshwe-panel {
  position: absolute; top: 14px; right: 14px; bottom: 14px; width: 372px;
  pointer-events: auto;
...
.dshwe-selrow { display: flex; gap: 8px; padding: 12px 16px 6px; align-items: center; }
.dshwe-sel {
...
.dshwe-sel:focus-visible { border-color: var(--dsw-alias-state-business-primary, #4176e6); }
.dshwe-addbtn {
...
.dshwe-hintline { display: flex; align-items: center; gap: 6px; padding: 4px 18px 8px; color: var(--dsw-alias-label-caption, #8a8a8a); font-size: 11px; }
.dshwe-tree { flex: 1; overflow: auto; padding: 2px 6px 14px; }
.dshwe-row {
...
.dshwe-insert { flex: none; display: inline-flex; align-items: center; color: var(--dsw-alias-state-business-primary, #4176e6); opacity: 0; transition: opacity .12s ease; }
.dshwe-row:hover .dshwe-insert, .dshwe-row:focus-visible .dshwe-insert { opacity: 1; }
...
.dshwe-row:focus-visible, .dshwe-icobtn:focus-visible, .dshwe-act:focus-visible, .dshwe-addbtn:focus-visible { outline: 2px solid var(--dsw-alias-state-business-primary, #4176e6); outline-offset: -2px; }
`)
    ...
    function Panel(props) {
      ...
      let body
      if (wsState.state === 'loading' && uniqOptions.length === 0) {
